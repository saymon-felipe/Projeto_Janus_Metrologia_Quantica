...
# Defaults ajustáveis pelo subcomando analyze.
# A versão anterior fazia permutação também no RandomForest com 300 árvores,
# o que pode virar dezenas de milhares de treinos e parecer travado em PCs comuns.
ANALYSIS_RF_TREES = 80
ANALYSIS_PERMUTE_MODE = "logreg"  # none | logreg | rf | all
...
def grouped_permutation_test(model, X, y, groups, observed_acc: float, n_perm: int, seed: int = 42, label: str = "") -> float:
    if n_perm <= 0:
        return np.nan
    ...
    for perm_i in range(n_perm):
        if label and (perm_i == 0 or (perm_i + 1) % step == 0 or (perm_i + 1) == n_perm):
            print(f"      permutação {perm_i + 1}/{n_perm} — {label}", flush=True)
        yp = y.copy()
...
def analyze_one_dataset(df: pd.DataFrame, dataset: str, postselect: bool, window: int, feature_mode: str, n_perm: int, seed: int) -> List[Dict]:
    ...
    models = {
        "RandomForest": RF(n_estimators=ANALYSIS_RF_TREES, max_depth=8, random_state=seed, class_weight="balanced", n_jobs=-1),
        "LogisticRegression": LR(max_iter=5000, class_weight="balanced", solver="liblinear"),
    }

    rows = []
    for model_name, model in models.items():
        print(f"[*] Modelo: dataset={dataset} | postselect={postselect} | model={model_name} | janelas={len(y)} | perm={n_perm if n_perm else 0}", flush=True)
        ...
        if n_perm:
            if ANALYSIS_PERMUTE_MODE == "all":
                should_permute = True
            elif ANALYSIS_PERMUTE_MODE == "logreg" and model_name == "LogisticRegression":
                should_permute = True
            elif ANALYSIS_PERMUTE_MODE == "rf" and model_name == "RandomForest":
                should_permute = True

        if should_permute:
            perm_p = grouped_permutation_test(
                model, X, y, groups, acc, n_perm=n_perm, seed=seed,
                label=f"{dataset}/post={postselect}/{model_name}"
            )
        else:
            perm_p = np.nan
            if n_perm and ANALYSIS_PERMUTE_MODE != "none":
                print(f"      pulando permutation test para {model_name} (modo={ANALYSIS_PERMUTE_MODE})", flush=True)
...
def safe_table_for_report(df: pd.DataFrame) -> str:
    """Renderiza tabela para o relatório sem exigir a dependência opcional tabulate."""
    try:
        return df.to_markdown(index=False)
    except Exception:
        # Fallback robusto: não fica tão bonito quanto Markdown, mas nunca quebra a análise.
        return "```\n" + df.to_string(index=False) + "\n```"

def cmd_analyze(args) -> int:
    global ANALYSIS_RF_TREES, ANALYSIS_PERMUTE_MODE
    ANALYSIS_RF_TREES = int(args.rf_trees)
    ANALYSIS_PERMUTE_MODE = str(args.permute_models)
    ...
    print(f"[*] Config análise: rf_trees={ANALYSIS_RF_TREES}, permute_models={ANALYSIS_PERMUTE_MODE}, n_perm={args.n_perm}", flush=True)
    ...
    if not shot_summary.empty:
        report_lines.append(safe_table_for_report(shot_summary))
    else:
    ...
    if not metrics.empty:
        ...
        report_lines.append(safe_table_for_report(m2))
    else:
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    an.add_argument("--n-perm", type=int, default=100, help="Número de permutações por teste")
    an.add_argument("--permute-models", choices=["none", "logreg", "rf", "all"], default="logreg", help="Quais modelos recebem permutation test. 'all' é muito lento.")
    an.add_argument("--rf-trees", type=int, default=80, help="Número de árvores do RandomForest na análise")
